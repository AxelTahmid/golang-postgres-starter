...
func NewProblemDetails(status int, detail, title, problemType string) *ProblemDetails {
	if problemType == "" {
		switch {
		case status == 404:
			problemType = GetProblemTypeURL("not_found_error")
		case status >= 400 && status < 500:
			problemType = GetProblemTypeURL("bad_request_error")
		case status >= 500:
			problemType = GetProblemTypeURL("server_error")
		default:
...
//
// This function allows applications using httpx to provide a custom domain and structure
